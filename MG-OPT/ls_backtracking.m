function [stp, f, options, work] = ls_backtracking(stp,f, g , options , work)
%--------------------------------------------------------------------------
%
...
% all subsequent backtracks, cubic fit
    ...
    if abs(coeff(1)) < deps || abs( -coeff(2) + sqrt(disc)) < deps
    %if abs(coeff(1)) == 0
    %if (coeff(1)) == 0        
...
% if stptemp > deps; stp = stptemp; else stp = deps; end
