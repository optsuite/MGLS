function print_mg_output(filename,pars, output, stat)
...
% for iter = pars.lev_coarest : pars.lev_finest
%    fprintf(fid,'%d &\t %d &\t %d &\t %d \\\\ \\hline \n', iter, stat.iter(iter), ...
%                                     stat.nfe(iter), stat.nhe(iter) ); 
% end
...
fprintf(fid,'}\\hline \n');
fprintf(fid,' & \\multicolumn{%d}{|c|}{a} \\\\ \\hline \n', item);
fprintf(fid,'\t h & \t');
...
fprintf(fid,'\n\n');
sstat = [(pars.lev_coarest: pars.lev_finest)',  stat.nfe, stat.nge, stat.n_cycle, stat.norm_g,stat.cpu];
fprintf(fid,'\\begin{tabular}[t]{|c|c|c|c|c|c|}\\hline \n');
fprintf(fid,'\\multicolumn{6}{|c|}{a} \\\\ \\hline \n');
fprintf(fid,'h & nfe & nge & nv & $||g^*||$ & CPU \\\\ \\hline \n');
fprintf(fid,'%d & %d & %d & %d & %3.1e & %6.2f \\\\ \\hline \n', sstat');
fprintf(fid,'\\end{tabular}\n');

fprintf(fid,'\n\nwithout n_cycle \n');
sstat = [(pars.lev_coarest: pars.lev_finest)',  stat.nfe, stat.nge, stat.norm_g,stat.cpu];
fprintf(fid,'\\begin{tabular}[t]{|c|c|c|c|c|}\\hline \n');
fprintf(fid,'\\multicolumn{5}{|c|}{a} \\\\ \\hline \n');
...
fprintf(fid,'\n\n MLS \n\n');
sstat = [(pars.lev_coarest: pars.lev_finest)',  stat.nfe, stat.nge, stat.n_cycle];
fprintf(fid,'\\begin{tabular}[t]{|c|c|c|c|}\\hline \n');
fprintf(fid,'\\multicolumn{4}{|c|}{a} \\\\ \\hline \n');
